...
import {
  ...
  Modal, Descriptions, Timeline, message, Avatar, Badge,
...
export const SupervisorDashboard = () => {
  ...
  // Fetch loads — supervisors monitor loads assigned to them
      ...
      render: (_, record) => (
        <Space size={4} wrap>
          {record.status === "IN_TRANSIT" && (
            <Tooltip title="Confirm unloading and mark delivered">
              <Button
                size="small"
                type="primary"
                style={{ background: "#52c41a", borderColor: "#52c41a" }}
                onClick={() => changeStatus(record, "DELIVERED", "Confirmed by supervisor")}
                loading={updating}
              >
                Confirm Delivered
              </Button>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} xl={7}>
          ...
          >
            ...
            />
            <Title level={4} style={{ margin: "0 0 4px" }}>{user.name || "Supervisor"}</Title>
